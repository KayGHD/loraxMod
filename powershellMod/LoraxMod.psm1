...
function Find-ASTPattern {
    <#
    .SYNOPSIS
    Generic pattern-based AST analysis using streaming parser

    .DESCRIPTION
    Analyzes source code for specific AST patterns using the loraxMod streaming
    parser's analyze action. Supports 37 categories across 12 languages.

    Core: function_calls, imports, closures, async_await, generators, decorators,
    exceptions, classes, functions, macros, comments, strings

    C++20: cpp_modules, cpp_concepts, cpp_reflection, cpp_memory

    OOP: structs, enums, typedefs, templates, access_specifiers, virtual_methods,
    operators, inheritance, friends, initializers

    Memory/Control: pointers, range_loops, conditionals, lambda_captures, static_asserts

    Language-specific: rust_errors, csharp_preprocessor, fortran_legacy,
    namespaces, html_elements, html_scripts

    .PARAMETER Language
    Programming language (c, cpp, csharp, python, javascript, bash, powershell, r, rust, css, fortran, html)

    .PARAMETER Category
    Analysis category to search for
    ...
    Optional filter - only return matches with these names

    .PARAMETER IncludeLocation
    Include line/column location info (default: true)

    .EXAMPLE
    Find-ASTPattern -Language python -Category function_calls -Code 'print(1); os.path.join(a,b)'
    Find function calls in Python code

    .EXAMPLE
    Find-ASTPattern -Language javascript -Category closures -FilePath app.js
    Find arrow functions and function expressions in JavaScript file

    .EXAMPLE
    Find-ASTPattern -Language csharp -Category classes -Code 'class Foo {} class Bar {}'
    Find class definitions in C# code

    .EXAMPLE
    Get-ChildItem *.py | ForEach-Object { Find-ASTPattern -Language python -Category imports -FilePath $_.FullName }
    Find all imports across Python files

    .NOTES
    Returns objects with: type, name, value, location, text
    Uses streaming parser analyze action for consistent cross-language support
    See 'categories' action for full list of supported categories per language
    #>
    [CmdletBinding(DefaultParameterSetName='Code')]
    param(
        [Parameter(Mandatory=$true)]
        [ValidateSet('c', 'cpp', 'csharp', 'python', 'javascript', 'bash', 'powershell', 'r', 'rust', 'css', 'fortran', 'html')]
        [string]$Language,

        [Parameter(Mandatory=$true, ParameterSetName='Code')]
        [Parameter(Mandatory=$true, ParameterSetName='File')]
        [Parameter(Mandatory=$true, ParameterSetName='CodeWithCategory')]
        [Parameter(Mandatory=$true, ParameterSetName='FileWithCategory')]
        [ValidateSet(
            'abstract', 'allocation', 'arguments', 'arrays', 'arrow_lambda',
            'assignment', 'async_await', 'async_stmt', 'async_yield', 'at_rules',
            'attributes', 'await', 'binary', 'binding', 'blocks',
            'break_continue', 'call', 'cast', 'catch_except', 'class_type',
            'classes', 'closures', 'commands', 'comments', 'comparison_boolean',
            'compound_stmt', 'comprehensions', 'conditional', 'cpp_concepts', 'cpp_modules',
            'csharp_preprocessor', 'custom_operators', 'declaration_lists', 'declaration_stmt', 'decorators',
            'destructuring', 'elements', 'else_elif', 'enums', 'error_propagation',
            'escape_sequences', 'exceptions', 'expression_stmt', 'expressions', 'ffi',
            'field', 'finally', 'formal_params', 'fortran_end_stmt', 'function_calls',
            'function_def', 'functions', 'generators', 'generic', 'goto_label',
            'heredocs', 'html_elements', 'if_else', 'import_export_stmt', 'imports',
            'inheritance', 'initializers', 'interpolation', 'io_stmt', 'jsx_attributes',
            'jsx_content', 'jsx_elements', 'legacy', 'lifetimes', 'loops',
            'macros', 'macros_preproc', 'matching', 'member_access', 'namespaces',
            'numbers', 'objects', 'other', 'other_stmt', 'pairs_fields',
            'parameter', 'pipelines', 'pointers', 'primitive', 'program_root',
            'qualifier', 'query_clauses', 'redirections', 'references', 'regex',
            'return', 'rules', 'rust_errors', 'selectors', 'special_blocks',
            'special_elements', 'special_identifiers', 'special_literals', 'special_params', 'specifier',
            'strings', 'subscript_index', 'substitutions', 'switch_case', 'try_catch',
            'type_def', 'typed_params', 'types', 'unary', 'unsafe',
            'variable', 'where_constraint'
        )]
        [string]$Category,

        [Parameter(Mandatory=$true, ParameterSetName='CodeWithFamily')]
        [Parameter(Mandatory=$true, ParameterSetName='FileWithFamily')]
        [ValidateSet(
            'control-flow', 'expressions', 'types', 'declarations',
            'patterns', 'oop', 'metaprogramming', 'clauses', 'literals',
            'parameters', 'concurrency', 'operators', 'memory', 'misc',
            'shell', 'fortran', 'css', 'html', 'jsx', 'rust', 'cpp'
        )]
        [string]$Family,

        [Parameter(Mandatory=$true, ParameterSetName='CodeWithWildcard')]
        [Parameter(Mandatory=$true, ParameterSetName='FileWithWildcard')]
        [string]$Wildcard,
    ...
    if (-not $Code) {
        Write-Error "No code provided. Use -Code or -FilePath parameter."
        return
    }
    ...
    if ($Wildcard) {
        $request.category = $Wildcard  # Analyzer handles wildcard expansion
    } elseif ($Family) {
        $request.category = @{ family = $Family }
    } else {
        $request.category = $Category
    }

    if ($FilterNames) {
        $request.options.filterNames = $FilterNames
    }

    # Get streaming parser path (one level up from powershellMod)
    $parserPath = (Join-Path (Split-Path $script:ModuleRoot -Parent) "bin/streaming_query_parser.js") -replace '\\', '/'

    # Send request to streaming parser
    $requestJson = $request | ConvertTo-Json -Compress -Depth 10

    try {
        $response = $requestJson | & node $parserPath 2>$null
        $result = $response | ConvertFrom-Json

        if ($result.success) {
            if (-not $result.supported) {
                Write-Warning "Category '$Category' not supported for language '$Language'. $($result.message)"
                return @()
            }
            return $result.matches
        } else {
            Write-Error "Analysis failed: $($result.error)"
            return @()
        }
    }
    catch {
        Write-Error "Failed to execute streaming parser: $_"
        return @()
    }
}
...
function Find-FunctionCalls {
    <#
    .SYNOPSIS
    Find function calls with context using pattern-based analysis

    .DESCRIPTION
    Extracts all function calls from source code using the streaming parser's
    analyze action. Supports 10 languages with automatic node type detection.
    ...
    .EXAMPLE
    Find-FunctionCalls -Language python -Code 'print(1); os.path.join("a", "b")'
    Find function calls in Python code

    .EXAMPLE
    Find-FunctionCalls -Language rust -Code 'println!("hi"); foo();'
    Find both function calls and macro invocations in Rust
    ...
    param(
        [Parameter(Mandatory=$true)]
        [ValidateSet('c', 'cpp', 'csharp', 'python', 'javascript', 'bash', 'powershell', 'r', 'rust', 'fortran')]
        [string]$Language,
    ...
    if ($FunctionNames) { $params.FilterNames = $FunctionNames }

    Find-ASTPattern @params
}

function Get-IncludeDependencies {
    <#
    .SYNOPSIS
    Parse include/import directives from source files

    .DESCRIPTION
    Extracts import/include statements using the streaming parser's analyze action.
    Supports multiple languages: C, C++, JavaScript, Python, Rust, C#, Fortran, CSS.

    .PARAMETER Language
    Programming language. Default: auto-detect from FilePath, or 'c' if Code provided
    ...
    .EXAMPLE
    Get-IncludeDependencies -Language python -Code 'import os; from pathlib import Path'
    Parse imports from Python code

    .EXAMPLE
    Get-IncludeDependencies -Language javascript -Code 'import foo from "bar";'
    Parse imports from JavaScript code

    .NOTES
    Returns objects with: type, name, value, location, text
    Uses streaming parser analyze action with 'imports' category
    #>
    [CmdletBinding()]
    param(
        [Parameter()]
        [ValidateSet('c', 'cpp', 'csharp', 'python', 'javascript', 'rust', 'fortran', 'css', 'bash', 'powershell')]
        [string]$Language,
